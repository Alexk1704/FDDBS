...
public final class NoGroupWithNonJoinOrNonJoinHorizontalResultSet extends FedResultSet {
    ...
    public NoGroupWithNonJoinOrNonJoinHorizontalResultSet(FedConnection fedConnection, Statement statement)
            throws FedException {
    ...
    protected FedException closeLocalResultSets() {
        ...
        if (occuredExceptions.size() > 0) {
            fedException = new FedException(String.format("%s: An error occurred while closing local result sets.%n%s",
                    ...
                    occuredExceptions.get(0).getMessage()));
