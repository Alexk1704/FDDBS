...
public final class NoGroupWithNonJoinAndNonJoinHorizontalResultSet extends FedResultSet {
    ...
    public NoGroupWithNonJoinAndNonJoinHorizontalResultSet(FedConnection fedConnection, Statement statement) throws FedException {
    ...
    protected void resolveStatement() throws FedException {
        ...
        if (hasFullyQualifiedAttributes()) {
            ...
            conditionStatement = String.format("SELECT %s FROM %s WHERE %s AND %s",
                    fqAttributesString, tableName, getFirstBinaryExpression(), getSecondBinaryExpression());
        } else {
            conditionStatement = String.format("SELECT * FROM %s WHERE %s AND %s",
                    tableName, getFirstBinaryExpression(), getSecondBinaryExpression());
        }
        localHorizontalResultSets = getLocalResultSetsHorizontal(table, conditionStatement);
