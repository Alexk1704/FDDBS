...
public final class NoGroupHorizontalResultSet extends FedResultSet {
    ...
    public NoGroupHorizontalResultSet(FedConnection fedConnection, Statement statement) throws FedException {
    ...
    protected void resolveStatement() throws FedException {
        String tableName = ((SelectNoGroupStatement) statement).getTableIdentifiers().get(0).getIdentifier();
        Table table = metadataManager.getTable(tableName);
        String queryStatement;
        if (hasFullyQualifiedAttributes()) {
            List<FullyQualifiedAttributeIdentifier> fqAttributes = ((SelectNoGroupStatement) statement)
                    .getFQAttributeIdentifiers();
